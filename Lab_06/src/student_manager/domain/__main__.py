...
GRADES: dict[int, tuple[float, ...]] = {
    1: (95, 91, 94, 93), 2: (81, 74, 79, 82), 3: (88, 83, 85, 84),
    4: (90, 86, 87, 89), 5: (98, 96, 95, 97), 6: (77, 70, 72, 75),
    7: (73, 68, 70, 74), 8: (85, 80, 82, 83), 9: (91, 89, 90, 92),
}
...
def main() -> None:
    ...
    print(f"  python_course in book: {python_course in book}; book[python_course] = {[str(g) for g in book[python_course]]}")
    ...
    for report in app.reporting.group_reports():
        print(f"  {report['code']:<8} студентів {report['students']}  середній {report['average']:.2f}  кращий: {report['best']}")
    ...
    for student_id in (7, 8):
        ...
        print(f"  {s.name.full:<16} avg {s.average:5.2f}  threshold: {app.grading.status_of(student_id):<18} "
