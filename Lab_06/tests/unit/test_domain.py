...
def test_grade_is_frozen_validated_and_ordered() -> None:
    ...
    assert [g.ects for g in (Grade(59), Grade(60), Grade(70), Grade(75), Grade(85), Grade(90))] == ["FX", "E", "D", "C", "B", "A"]
...
def test_gradebook_is_a_collection() -> None:
    ...
    assert [(c.title, g.points) for c, g in book] == [("Професійний Python", 95), ("Професійний Python", 85), ("Мова С", 50)]
...
def test_reporting(app: tuple[Container, RecordingNotifier]) -> None:
    ...
    for student_id, points in ((1, 95), (2, 70), (3, 95)):
        container.enrollment.admit(Student(student_id, PersonName("Ім'я", f"Прізвище{student_id}"), GroupCode("ФЕП-31с")))
    ...
    assert report == {"id": 2, "name": "Прізвище2 Ім'я", "group": "ФЕП-31с", "courses": 1, "grades": 1,
                      "average": 70.0, "credits": 4, "status": AcademicStatus.GOOD}
    (group_report,) = container.reporting.group_reports()
    assert group_report == {"code": "ФЕП-31с", "students": 3, "average": pytest.approx(86.67, abs=0.01), "best": "Прізвище1 Ім'я"}
...
def test_composed_parts_are_testable_alone() -> None:
    ...
    assert TableFormatter().format({"student_id": 1, "course_id": 10, "points": 61}, Grade(61)) == "|   1 |  10 |  61.0 | D  |"
