...
def parse_id(row: Row, line_number: int, field: str = "student_id") -> int:
    ...
    try:
        ...
        raise RecordValidationError(f"{field} має бути цілим числом, отримано {text_of(row.get(field))!r}",
                                    line_number=line_number, field=field) from error
    if value <= 0:
        raise RecordValidationError(f"{field} має бути додатним, отримано {value}", line_number=line_number, field=field)
    return value
...
def parse_grade(value: object, line_number: int, field: str = "grade") -> float:
    ...
    try:
        ...
        raise RecordValidationError(f"{field} має бути числом, отримано {text_of(value)!r}",
                                    line_number=line_number, field=field) from error
    if not GRADE_MIN <= grade <= GRADE_MAX:
        raise RecordValidationError(f"{field} має бути в межах {GRADE_MIN:g}..{GRADE_MAX:g}, отримано {grade:g}",
...
def parse_student_row(row: Row, line_number: int) -> StudentRow:
    ...
    if len(name.split()) < 2:
        raise RecordValidationError(f"name має містити прізвище та ім'я, отримано {name!r}",
                                    line_number=line_number, field="name")
