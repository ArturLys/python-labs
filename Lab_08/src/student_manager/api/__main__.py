...
SEED = [
    {"first_name": "Марта", "last_name": "Гнатишин", "group": "ФЕП-31с", "average_grade": 93.4, "email": "marta@lnu.edu.ua"},
...
async def demo(db_path: Path) -> None:
    ...
    async with app.router.lifespan_context(app), httpx.AsyncClient(transport=transport, base_url="http://api") as http:
        ...
        await call(http, "POST", "/students", json={"first_name": "Ігор", "last_name": "Новак", "group": "феп31",
                                                     "average_grade": 150})
        ...
        await call(http, "GET", "/groups/statistics", params=[("code", "ФЕП-31с"), ("code", "ФЕП-32"), ("code", "ФЕП-41"),
                                                              ("limit", 2)])
    ...
    try:
        async with StudentApiClient("http://127.0.0.1:8765", timeout=0.3, attempts=3, base_delay=0.05,
                                    concurrency=2) as client:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    serve.add_argument("--db", default="sqlite:///data/students.db")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "serve":
        uvicorn.run(create_app(database_url=args.db, include_debug=True), host=args.host, port=args.port)
    else:
