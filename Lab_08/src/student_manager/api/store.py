...
from student_manager.db import (
    ConstraintViolationError, GroupRecord, GroupRepository, RecordNotFoundError, StudentRecord, StudentRepository,
    session_scope,
)
...
class StudentStore:
    ...
    def search(self, *, last_name: str | None = None, group: str | None = None, page: int = 1, size: int = 20,
               sort: SortKey = "id") -> StudentPage:
        with self._transaction() as session:
            ...
            order = {
                "id": StudentRecord.id, "last_name": StudentRecord.last_name,
                "grade": StudentRecord.average_grade, "-grade": StudentRecord.average_grade.desc(),
            }[sort]
    ...
    def create(self, data: StudentCreate) -> StudentOut:
        with self._transaction() as session:
            ...
            record = StudentRepository(session).add(StudentRecord(
                first_name=data.first_name, last_name=data.last_name, average_grade=data.average_grade,
                email=data.email, group=group,
            ))
    ...
    def group_statistics(self, code: str) -> GroupStatisticsOut:
        with self._transaction() as session:
            ...
            return GroupStatisticsOut(code=stats.code, students=stats.students, average=stats.average,
