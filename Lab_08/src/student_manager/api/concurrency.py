...
async def gather_statistics(source: StatisticsSource, codes: Iterable[str], *, limit: int = 3,
                            timeout: float = 2.0) -> list[GroupStatisticsOut]:
...
async def retry(operation: Callable[[], Awaitable[T]], *, attempts: int = 3, base_delay: float = 0.2,
                retry_on: tuple[type[BaseException], ...] = (Exception,), label: str = "operation") -> T:
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            logger.warning("%s: спроба %d/%d невдала (%s), повтор через %.2f с", label, attempt, attempts, reason, delay)
