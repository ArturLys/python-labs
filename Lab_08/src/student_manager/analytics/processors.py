...
def sort_items(records: Iterable[GradeRecord], key: KeyFunc | Callable[[GradeRecord], float],
               reverse: bool = False) -> list[GradeRecord]:
...
def students_per_group(records: Iterable[GradeRecord]) -> dict[str, int]:
    ...
    return {
        group: len({r.student for r in members})
        for group, members in group_by(records, lambda r: r.group).items()
    }
