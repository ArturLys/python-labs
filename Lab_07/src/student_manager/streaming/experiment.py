...
def format_table(results: list[Measurement]) -> str:
    header = (f"{'rows':>9} {'eager time':>11} {'lazy time':>11} {'eager peak':>11} {'lazy peak':>11} "
              f"{'eager 1st':>11} {'lazy 1st':>11}")
    lines = [header, "-" * len(header)]
    for m in results:
        lines.append(f"{fmt_int(m.rows):>9} {fmt_seconds(m.eager_time):>11} {fmt_seconds(m.lazy_time):>11} "
