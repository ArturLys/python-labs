...
def strict_vs_tolerant(data_dir: Path, *, valid: int = 1_000, invalid: int = 10) -> list[PolicyRun]:
    ...
    for policy in (STRICT, TOLERANT):
        ...
        runs.append(PolicyRun(policy.mode, statistics.total, statistics.valid, statistics.invalid,
                              exported, outcome, output.exists()))
    return runs
...
def format_memory_table(runs: list[MemoryRun]) -> str:
    lines = [f"{'records':>9} {'eager, s':>10} {'stream, s':>10} {'eager MiB':>11} {'stream MiB':>11} {'ratio':>7}", "-" * 63]
    lines += [
