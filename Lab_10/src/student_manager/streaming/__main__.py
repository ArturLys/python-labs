...
def main() -> None:
    ...
    show(
        f"НАБІР ДАНИХ: {path} — {'знайдено' if existed else 'згенеровано'}, "
    ...
    print(
        f"  рядків даних: {fmt_int(report.seen)}; валідних: {fmt_int(report.accepted)}; "
        f"відхилено: {fmt_int(sum(report.rejected.values()))}"
    )
    for reason, number in sorted(report.rejected.items()):
        print(f"    {reason:<15} {number}")
    print(
        f"  із балом >= {MINIMUM:g}: {fmt_int(stats.count)} записів; середній {stats.average:.2f}; "
    ...
    print(
        f"  записів у ланцюжку з балом >= {MINIMUM:g}: {fmt_int(sum(1 for _ in build_multi_pipeline(parts, MINIMUM)))}"
    )
    ...
    print(
        f"  lazy find_first: {fmt_seconds(early_result.lazy_time):>9}, "
        f"прочитано {fmt_int(early_result.lines_pulled)} рядків -> {fmt(early_result.lazy_hit)}"
    )
    print(
        f"  eager список:    {fmt_seconds(early_result.eager_time):>9}, "
