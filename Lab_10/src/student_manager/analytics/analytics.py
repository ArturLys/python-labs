...
def student_averages(records: Iterable[GradeRecord]) -> dict[str, float]:
    ...
    return {student: mean_of(*(r.grade for r in rows)) for student, rows in index_by_student(records).items()}


def group_averages(records: Iterable[GradeRecord]) -> dict[str, float]:
    return {group: mean(r.grade for r in members) for group, members in group_by(records, lambda r: r.group).items()}
