...
def main(argv: list[str] | None = None) -> None:
    ...
    print(
        f"  контроль через statistics/NumPy: mean={check.mean():.4f} std={check.std():.4f} -> збігається: "
    ...
    print(
        f"  to_arrays (list -> ndarray): peak {fmt_bytes(peak_arrays)}; масиви: {fmt_bytes(arrays.grades.nbytes + arrays.group_ids.nbytes)}"
    )
    print(f"  compute_stats_numpy: peak {fmt_bytes(peak_numpy)}")
    print(
        f"  list[Student] (оцінка, без рядків): {fmt_bytes(list_bytes)} проти ndarray grades {fmt_bytes(arrays.grades.nbytes)}"
    )
    ...
    print(
        f"  to_columns (об'єкти -> списки float/int): {measure('c', partial(to_columns, students), repeats=3).best:.4f} s"
    )
    timings = [t_base]
    for workers in (1, 2, 4, 8):
        timings.append(
            measure(f"ThreadPoolExecutor x{workers}", partial(stats_threads, students, workers), repeats=args.repeats)
        )
    timings.append(
        measure("ProcessPool x4, об'єкти, новий пул", partial(stats_processes, students, 4), repeats=args.repeats)
    )
    timings.append(
        measure(
            "ProcessPool x4, колонки, новий пул", partial(stats_processes_columns, columns, 4), repeats=args.repeats
        )
    )
    with ProcessPoolExecutor(max_workers=8) as warm:
        ...
        for workers in (1, 2, 4, 8):
            timings.append(
                measure(
                    f"ProcessPool x{workers}, колонки, теплий пул",
                    partial(stats_processes_columns, columns, workers, warm),
                    repeats=args.repeats,
                )
            )
    timings.append(
        measure("NumPy (з конвертацією)", partial(compute_stats_numpy_from_students, students), repeats=args.repeats)
    )
    ...
    rows += [
        {
            ...
            "best": round(t.best, 5),
            "mean": round(t.mean, 5),
            "speedup": round(t.speedup(t_base), 3),
        }
        for t in timings
    ]
    ...
    with ProcessPoolExecutor(max_workers=4) as warm:
        ...
        for size in (10_000, 50_000, 200_000, 500_000):
            ...
            rows += [
                {"size": size, "method": m, "best": round(t, 5), "mean": "", "speedup": round(tb / t, 3)}
    ...
    print(
        f"  cold (miss): {t_cold * 1000:.2f} ms; warm (hit): {t_warm * 1e6:.1f} µs; speedup {t_cold / t_warm:,.0f}×".replace(
            ",", " "
        )
    )
    ...
    for interval in (None, 1e-6):
        ...
        print(
            f"  switch interval {label:<15} без Lock: {unsafe:>7} із 800000 (втрачено {800_000 - unsafe:>6}); з Lock: {safe}"
        )
