...
def describe(config: AppConfig, base: Path) -> None:
    ...
    print(
        f"  output: path={rel(config.output.path)}  errors_path={rel(config.output.errors_path)}  "
        f"summary_path={rel(config.output.summary_path)}  backup={config.output.backup}"
    )
    print(
        f"  processing: minimum_grade={config.processing.minimum_grade:g}  "
        f"skip_invalid={config.processing.skip_invalid}  progress_every={config.processing.progress_every}"
    )
    print(
        f"  logging: level={config.logging.level}  path={rel(config.logging.path)}  "
...
def rows_then_disk_failure() -> Iterator[Row]:
    ...
    yield {
        ...
        "group": "ФЕП-31с",
        "grade": 93.25,
        "email": "marta.hnatyshyn@lnu.edu.ua",
    }
...
def demo(data_dir: Path) -> None:
    ...
    overridden = load_config(
        config_path, environ={"STUDENT_MANAGER_MINIMUM_GRADE": "90", "STUDENT_MANAGER_SKIP_INVALID": "no"}
    )
    print(
        f"  з environment (MINIMUM_GRADE=90, SKIP_INVALID=no): minimum_grade={overridden.processing.minimum_grade:g}, "
    ...
    print(
        f"  students.json незмінний: {config.output.path.read_bytes() == before}; "
    ...
    bad_row: Row = {
        ...
        "group": "ФЕП-31с",
        "grade": "wrong",
        "email": "andrii.lytvyn@lnu.edu.ua",
    }
    try:
    ...
    print(
        f"  students.json незмінний: {config.output.path.read_bytes() == before}; .tmp існує: {tmp.exists()}; .bak існує: {bak.exists()}"
    )
    rows = [row for _, row in read_rows(config.output.path)]
    write_rows(rows, config.output.path, backup=True)
    print(
        f"  повторний експорт {len(rows)} записів із backup=True: .bak існує: {bak.exists()}, збігається з попереднім: {bak.read_bytes() == before}"
    )
    ...
    for name in ("grades.csv", "grades.jsonl", "grades.yaml", "grades.json.gz"):
        ...
        print(
            f"  {name:<14} записано {written}, прочитано {len(back)}, {path.stat().st_size:>5} байт, "
    ...
    (data_dir / "broken.json").write_text(
        '[{"student": "Гнатишин Марта", "group": "ФЕП-31с", "discipline": "Мова С", "grade": 91}, {"student": ', "utf-8"
    )
    (data_dir / "broken.yaml").write_text("- student: Гнатишин Марта\n  grade: [91, 92\n", "utf-8")
    (data_dir / "wrong_header.csv").write_text("id,name\n1,x\n", "utf-8")
    for path in (
        data_dir / "broken.json",
        data_dir / "broken.yaml",
        data_dir / "wrong_header.csv",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m student_manager.storage", description="Імпорт/експорт і конфігурація (ЛР5)"
    )
    parser.add_argument(
        "--data-dir", type=Path, default=Path("data"), help="каталог для файлів демонстрації (типово data/)"
    )
