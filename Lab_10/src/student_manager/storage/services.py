...
def import_records(
    path: Path,
    parse: Callable[[Row, int], T],
    *,
    required: tuple[str, ...],
    policy: ImportPolicy,
    statistics: ImportStatistics,
    on_invalid: InvalidCallback | None = None,
) -> Iterator[T]:
...
def import_students(
    path: Path, *, policy: ImportPolicy, statistics: ImportStatistics, on_invalid: InvalidCallback | None = None
) -> Iterator[StudentRow]:
    return import_records(
        path, parse_student_row, required=STUDENT_FIELDS, policy=policy, statistics=statistics, on_invalid=on_invalid
    )
...
def error_report(path: Path) -> Iterator[InvalidCallback]:
    ...
    with AtomicWriter(path) as file:
        ...
        def record(error: RecordValidationError, raw: Row) -> None:
            writer.writerow(
                {"line": error.line_number, "field": error.field, "message": str(error), "raw": to_json(raw)}
            )
...
def run_pipeline(config: AppConfig) -> PipelineResult:
    ...
    try:
        ...
        logger.info(
            "підсумок: %s exported=%d (minimum_grade=%g, режим %s)",
            statistics,
            exported,
            config.processing.minimum_grade,
            policy.mode,
        )
