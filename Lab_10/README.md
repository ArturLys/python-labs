...
## Лабораторна робота №9 — паралельність, профілювання, оптимізація

Пакет `student_manager.perf`: `dataset.py` (синтетичні студенти), `baseline.py` (Python-цикли + перевірка
рівності результатів), `parallel.py` (ThreadPoolExecutor, ProcessPoolExecutor, threading.Thread + Lock,
демонстрація race condition), `vectorized.py` (NumPy), `cache.py` (lru_cache з версією даних),
`profiling.py` (timeit, cProfile, tracemalloc).

```bash
pip install -e ".[perf]"
python -m student_manager.perf --size 200000 --repeats 5   # повний експеримент, результати у data/benchmark.csv
pytest tests/unit/test_perf.py -v
```

## Лабораторна робота №10 — production: пакет, конфігурація, Docker, CI

[![CI](https://github.com/ArturLys/student-manager/actions/workflows/ci.yml/badge.svg)](https://github.com/ArturLys/student-manager/actions/workflows/ci.yml)

### Пакет

```bash
python -m build                      # dist/student_manager-1.0.0-py3-none-any.whl + .tar.gz
pip install "student-manager[api] @ file://$(pwd)/dist/student_manager-1.0.0-py3-none-any.whl"
student-api serve                    # читає .env / змінні середовища
```

### Конфігурація через середовище (`.env.example`)

| Змінна | Типово | Призначення |
|--------|--------|-------------|
| `DATABASE_URL` | `sqlite:///data/students.db` | SQLAlchemy URL бази |
| `APP_ENV` | `development` | `development` / `test` / `production` (у production `/debug` вимкнено, `API_TOKEN` обов'язковий) |
| `LOG_LEVEL` | `INFO` | рівень логування |
| `API_HOST`, `API_PORT` | `127.0.0.1`, `8000` | адреса сервера |
| `API_TOKEN` | — | **секрет**; коли задано, POST/PATCH/DELETE потребують заголовка `X-API-Token` |

Секрети живуть лише в середовищі або у локальному `.env` (у `.gitignore`); у код і в Git потрапляє тільки `.env.example`.

### Docker

```bash
docker build -t student-manager:1.0.0 .
docker run -d -p 8000:8000 -e API_TOKEN=change-me -e LOG_LEVEL=DEBUG -v students-data:/data student-manager:1.0.0
curl http://localhost:8000/health       # {"status":"ok","version":"1.0.0","app_env":"production","database":"ok"}
docker compose up --build               # те саме з .env і volume
```

Образ — multi-stage (wheel збирається в builder), non-root користувач `app`, `HEALTHCHECK` на `/health`,
міграції Alembic виконуються при старті контейнера.

### Приклади API

```bash
curl http://localhost:8000/students?group=ФЕП-31с&sort=-grade
curl -X POST http://localhost:8000/students -H "Content-Type: application/json" -H "X-API-Token: change-me" \
     -d '{"first_name":"Марта","last_name":"Гнатишин","group":"ФЕП-31с","average_grade":93.4}'
curl http://localhost:8000/groups/ФЕП-31с/students
```

### CI (GitHub Actions, `.github/workflows/ci.yml`)

`ruff check` + `ruff format --check` → `mypy --strict` → `pytest` на Python 3.12 і 3.13 з branch coverage ≥ 80 % →
`python -m build` + `twine check` + встановлення wheel у чисте середовище → `docker build`, запуск контейнера,
`curl /health` і перевірка, що POST без токена дає 401.

### Production checklist

- [x] src-layout, `pyproject.toml` з метаданими, семантична версія `1.0.0`, `CHANGELOG.md`
- [x] wheel і sdist збираються та проходять `twine check`; консольні команди `student-api`, `student-storage`, `student-manager`
- [x] конфігурація лише через середовище; `.env` у `.gitignore`, `.env.example` у репозиторії; секрети маскуються в логах
- [x] `/health`, структуроване логування з `LOG_LEVEL`, захист мутацій токеном
- [x] Dockerfile multi-stage + non-root + healthcheck, `.dockerignore`, docker-compose з volume
- [x] CI: lint, format, типи, тести на двох версіях Python, покриття, build, Docker
