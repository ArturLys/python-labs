...
@pytest.mark.parametrize(
    "name", ["grades.csv", "grades.json", "grades.jsonl", "grades.yaml", "grades.jsonl.gz", "grades.csv.gz"]
)
...
def test_custom_json_encoder() -> None:
    ...
    assert (
        to_json(payload)
        == '{"p": "x", "s": [1, 2], "c": {"minimum_grade": 60, "skip_invalid": true, "progress_every": 0}}'
    )
...
def test_configuration_errors(
    config_path: Path, old: str, new: str, message: str, cause: type[BaseException] | None
) -> None:
...
def test_strict_vs_tolerant_experiment(tmp_path: Path) -> None:
    ...
    assert (tolerant.total, tolerant.valid, tolerant.invalid, tolerant.exported, tolerant.output_exists) == (
        55,
        50,
        5,
        50,
