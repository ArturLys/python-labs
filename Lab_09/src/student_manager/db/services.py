    ...
    def add(self, data: NewStudent) -> int:
        with self._transaction() as session:
            ...
            record = StudentRepository(session).add(StudentRecord(
                ...
                average_grade=data.average_grade, email=data.email, group=group,
            ))
            return record.id
    ...
    def import_many(self, rows: Iterable[NewStudent]) -> int:
        ...
        with self._transaction(on_row=lambda: added + 1) as session:
            for row in rows:
                ...
                StudentRepository(session).add(StudentRecord(
                    first_name=row.first_name, last_name=row.last_name, average_grade=row.average_grade,
                    email=row.email, group=group,
                ))
                added += 1
        return added
