...
class StudentApiClient:
    def __init__(self, base_url: str = "http://127.0.0.1:8000", *, timeout: float = 5.0, attempts: int = 3,
                 base_delay: float = 0.2, concurrency: int = 4, transport: httpx.AsyncBaseTransport | None = None) -> None:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> httpx.Response:
        ...
        response = await retry(once, attempts=self._attempts, base_delay=self._base_delay,
                               retry_on=(httpx.TransportError, RetryableStatusError), label=f"{method} {path}")
