...
from student_manager.perf import (
    GroupStatisticsService, compute_stats, compute_stats_numpy_from_students, make_students, measure, peak_memory,
    profile_top, race, stats_close, stats_processes, stats_raw_threads, stats_threads,
)
from student_manager.perf.parallel import (
    merge, partial_stats, partial_stats_columns, split, stats_processes_columns, to_columns,
)
...
@pytest.mark.parametrize("implementation", [
    partial(stats_threads, workers=3), partial(stats_raw_threads, workers=3), compute_stats_numpy_from_students,
...
def test_columns_variant_matches_baseline_in_process(students: list[Student]) -> None:
    ...
    parts = [partial_stats_columns(columns.grades[o:o + len(c)], columns.group_ids[o:o + len(c)], o) for o, c in pieces]
